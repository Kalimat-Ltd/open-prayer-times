...
1. **Detect reference clock-shift artifacts** — scan the reference data for date ranges where all prayers are systematically shifted by the same constant amount (the detector checks for uniform offsets across all six prayers simultaneously). These dates are excluded from core fitting to avoid biasing the angle search toward a compromise that is wrong for both shifted and unshifted periods. The detector is DST-aware: if a `tz_name` (IANA timezone name) is provided, any detected shift window that aligns with a genuine DST transition for that timezone is suppressed — preventing DST from being double-counted as a "clock-shift artifact" when the calculator already accounts for it natively via `pytz`. More broadly, setting `reference_year` correctly is the best first step: when the calculator resolves DST using the reference data's actual year, most DST-driven apparent shifts disappear before the detector even runs
